<template>
  <div>
    <div class="container list-menu">
      <div class="row">
        <div class="col-md-12" v-for="(data, i) in product" :key="`A-${i}`" @click="handleDetail(data.id)">
          <div class="wrapper-product">
            ...
            <div class="content-product d-flex flex-column justify-content-between">
              <div class="content-top">
                <h6>{{data.name}}</h6>
                <div v-if="data.productCategoryId">
                  <p class="category-name">{{data.ProductCategory.name}}</p>
                </div>
                <div v-else>
...
export default {
  ...
  methods: {
    async filterCategory () {
      this.resultProduct.map(value => {
        if (value.productCategoryId) {
          if (value.ProductCategory.name.toLowerCase() === this.categoryName) {
    ...
    handleDetail (id) {
      this.$router.push({ name: 'DetailNoscan', params: { id } })
    }
  },
  watch: {
    $route (to) {
